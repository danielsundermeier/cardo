        ...
        <div class="row">
            <div class="col d-flex align-items-start mb-1 mb-sm-0">
                <div class="form-group mr-1" style="margin-bottom: 0;">
                    <select class="form-control" :class="'course_id' in errors ? 'is-invalid' : ''" v-model="form.course_id">
                        <option :value="null">Kurs wählen</option>
                        <option :value="course.id" v-for="course in availableCourses">{{ course.name }}</option>
                    </select>
                    <div class="invalid-feedback" v-text="'course_id' in errors ? errors.course_id[0] : ''"></div>
                </div>
...
<script>
    ...
    export default {
        ...
        data () {
            return {
                uri: '/partner/' + this.model.id + '/participant',
                items: [],
                isLoading: true,
                filter: {
